<script>
	...
	$: finalWinner = bracket && bracket.status === 'completed' 
		? bracket.matches
			.filter(m => m.round === bracket.total_rounds)
			.find(m => m.winner_id)?.winner_id
		: null;
	$: finalWinnerItem = finalWinner && bracket 
	...
	async function selectWinner(match, winnerId) {
		...
		try {
			bracket = await api.updateMatch(bracket.id, match.id, { winner_id: winnerId });
		} catch (e) {
	...
	async function clearWinner(match) {
		...
		if (!confirm('Are you sure you want to clear this match winner? This will also remove any subsequent rounds that depend on this match.')) {
			return;
		}

		updatingMatch = match.id;
		try {
			bracket = await api.clearMatchWinner(bracket.id, match.id);
		} catch (e) {
	...
	async function resetBracket() {
		if (!bracket || !confirm('Are you sure you want to reset this bracket? All progress will be lost.')) {
			return;
		}
							...
							<div class="matches">
								{#each leftMatches as match}
									<div class="match">
										...
										{/if}
										<div
											class="match-item {match.item1 && isItemEliminated(match.item1.id) ? 'eliminated' : ''} {match.winner_id === match.item1?.id ? 'winner' : ''} {hoveredItemId === match.item1?.id ? 'hovered' : ''}"
											on:click={() => match.item1 && !match.winner_id && selectWinner(match, match.item1.id)}
											on:mouseenter={() => match.item1 && !match.winner_id && (hoveredItemId = match.item1.id)}
										...
										<div class="vs">vs</div>
										<div
											class="match-item {match.item2 && isItemEliminated(match.item2.id) ? 'eliminated' : ''} {match.winner_id === match.item2?.id ? 'winner' : ''} {hoveredItemId === match.item2?.id ? 'hovered' : ''}"
											on:click={() => match.item2 && !match.winner_id && selectWinner(match, match.item2.id)}
											on:mouseenter={() => match.item2 && !match.winner_id && (hoveredItemId = match.item2.id)}
											on:mouseleave={() => hoveredItemId = null}
					...
					<div class="bracket-column bracket-final">
						<div class="round final-round">
							...
							<div class="match">
								...
								{/if}
								<div
									class="match-item {finalMatch.item1 && isItemEliminated(finalMatch.item1.id) ? 'eliminated' : ''} {finalMatch.winner_id === finalMatch.item1?.id ? 'winner' : ''} {hoveredItemId === finalMatch.item1?.id ? 'hovered' : ''}"
									on:click={() => finalMatch.item1 && !finalMatch.winner_id && selectWinner(finalMatch, finalMatch.item1.id)}
									on:mouseenter={() => finalMatch.item1 && !finalMatch.winner_id && (hoveredItemId = finalMatch.item1.id)}
									on:mouseleave={() => hoveredItemId = null}
								...
								<div class="vs">vs</div>
								<div
									class="match-item {finalMatch.item2 && isItemEliminated(finalMatch.item2.id) ? 'eliminated' : ''} {finalMatch.winner_id === finalMatch.item2?.id ? 'winner' : ''} {hoveredItemId === finalMatch.item2?.id ? 'hovered' : ''}"
									on:click={() => finalMatch.item2 && !finalMatch.winner_id && selectWinner(finalMatch, finalMatch.item2.id)}
									on:mouseenter={() => finalMatch.item2 && !finalMatch.winner_id && (hoveredItemId = finalMatch.item2.id)}
									on:mouseleave={() => hoveredItemId = null}
